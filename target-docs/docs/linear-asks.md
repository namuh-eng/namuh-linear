...
Web forms | On Enterprise plans, anyone in your identity provider can submit Asks with web forms.   


Users will need to log in to your Asks domain in order to submit a web form. After submission, users receive an email. This corresponds with a synced thread in the associated issue, so the requester can follow up and receive updates on their request.

> [!NOTE]
> Asks with email is not designed to be used for frontline customer support. Features common in dedicated customer support tools like first response times and NPS scores are not planned.

### Linear Asks vs Advanced Linear Asks

Asks features are available in both Business and Enterprise plans, with additional features available to Enterprise workspaces:
...
> [!NOTE]
> Web forms are supported on Enterprise plans.
...
Requesters receive an email acknowledgement when their form is submitted. Comments on the Linear issue sync both ways with the email thread, so you Linear users can respond to the requester without leaving Linear and email replies will appear as new comments.

Your top-level URL presents employees with all pages you’ve set up — usually by team or topic — so users can easily find the area they want to submit forms for. Each page holds individual forms, which correspond to templates in your workspace.

### Configure 

Workspace owners can enable web forms from [settings](https://linear.app/settings/asks). Click `+` next to _Web_ to get started. 

#### Create your domain

Choose whether to use the Linear provided URL or host your web forms on a custom domain. Hosting through Linear offers simpler setup, but you may wish to use a custom domain to make Asks feel more native to your organization.

_If you switch hosting between a Linear hosted domain and a custom domain later, you will need to edit your SAML app since redirect URLs will change._

If you choose to use the Linear provided domain, please skip to the _Set up SAML authentication_ step below. 

#### Set a custom email address

If you use a custom domain, you’ll need to set an email address for sending issue updates and notifications to submitters. When users receive an email or updates after submitting a form, those emails will come from this address. 

#### DNS setup

Add DNS records in your domain provider to verify your domain and enable email delivery. You can proceed to the next step without waiting for verification to complete, as this may take up to 48 hours. After verification completes, you’ll see a green _Enabled_ message underneath _Web_ in [Asks settings](https://linear.app/settings/asks).

#### Email forwarding

Email forwarding allows replies sent by the form submitter to properly appear in the issue’s synced thread in Linear. Learn more about how to set up email forwarding below.
...
1. In [Asks](https://linear.app/settings/asks) settings, click `+` next to _Web_ to begin SAML setup. This will provide you with the details required to create an SAML app for Asks web forms in your identify provider.
2. Open your identity provider and configure a SAML app with the details provided. _If your identity provider supports importing metadata instead of copy/pasting values, you can download that file from Linear for easier setup_. Once all provided values are present in your SAML app, create it.
...
Pages organize your forms by thematic group, and help direct employees to the appropriate webform. When an employee opens your Asks website, they’ll see a list of pages.
...
Create new pages in Asks settings. When creating a page, you’ll set its name, description and URL. This is also where you choose the templates to provide to the page, and optionally customize the format of email replies the submitting user receives.
...
Once you’ve created the template, make sure to add it to a page (if you don’t, it will be available in Linear, not Asks.) Once added, users will be able to submit the form from web.
