...
This feature allows you to connect Jira spaces to Linear teams, so that new issues and projects created in Jira or Linear are kept current in both places. Jira Sync is a forward looking integration — if you wish to import existing issues from Jira as well, please see our [import](https://linear.app/docs/jira-to-linear) guide.
