...
describe("TeamTriagePage UI", () => {
  ...
  it("accepts a triage issue", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation((url) => {
        if (url.toString().includes("/api/teams/ENG/triage") && !url.toString().includes("/iss-1")) {
    ...
    const acceptButtons = screen.getAllByRole("button", { name: "Accept issue" });
    // ENG-1 is index 1 because of default created-desc sort
    fireEvent.click(acceptButtons[1]);

    await waitFor(() => {
        const calls = fetchSpy.mock.calls;
        const patchCall = calls.find(call => call[0].toString().includes("/api/teams/ENG/triage/iss-1"));
  ...
  it("declines a triage issue", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation((url) => {
        if (url.toString().includes("/api/teams/ENG/triage") && !url.toString().includes("/iss-2")) {
            return Promise.resolve({
                ok: true,
                json: async () => mockTriageData,
            } as Response);
    ...
    const declineButtons = screen.getAllByRole("button", { name: "Decline issue" });
    // ENG-2 is index 0
    fireEvent.click(declineButtons[0]);

    await waitFor(() => {
        const calls = fetchSpy.mock.calls;
        const patchCall = calls.find(call => call[0].toString().includes("/api/teams/ENG/triage/iss-2"));
        expect(patchCall).toBeDefined();
        if (patchCall) {
          expect(patchCall[1]).toMatchObject({
            method: "PATCH",
