...
describe("TeamTemplatesSettingsPage integration", () => {
  ...
  const mockTemplatesData = {
    ...
    templates: [
      { id: "temp-1", name: "Backend Project", description: "Standard API project setup" },
      { id: "temp-2", name: "Frontend Sprint", description: "React component library template" }
    ],
  };
  ...
  it("shows an empty state when the team has no templates", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ team: { name: "Empty Team" }, templates: [] }),
    }));

    render(<TeamTemplatesSettingsPage />);

    await waitFor(() => {
      expect(screen.getByText("No templates have been created for this team.")).toBeInTheDocument();
    });
  });
});
